...
const ParkingOffer: React.FC = () => {
  ...
  const parkingOffer: FullParkingOffer = useSelector((state: RootState) =>
    fullParkingsOffersWithIdSelector(state, params.id)
  );
  const [parkingOfferToUpdate, setParkingOfferToUpdate] =
    useState<FullParkingOffer>(parkingOffer);
  ...
  const acceptOffer = () => {
    editParkingOffer(parkingOfferToUpdate.id, {
      slots: parkingOfferToUpdate.slots,
    })
      .then((res) => {
  ...
  return (
    <IonPage>
      ...
      <IonContent fullscreen>
        <ParkingDetails
          parking={parkingOffer?.parking}
          isRouting={true}
          isCanAddComment={boolean(canAddComment)}
          isCanAddImage={boolean(canAddImage)}
        />
        <div color="primary" className={"parking-offer-details"}>
          <IonGrid className={"parking-grid"}>
            <IonRow>
              <IonCol>
                <IonText color="primary">סטטוס</IonText>
              </IonCol>
              <IonCol>
                {parkingOffer.status == "Open" && (
                  <IonText color="primary">פנויה</IonText>
                )}
                {parkingOffer.status == "Closed" && (
                  <IonText color="primary">תפוסה</IonText>
                )}
              </IonCol>
            </IonRow>
            <IonRow>
              <IonCol>
                <IonText color="primary">תאריך התחלה</IonText>
              </IonCol>
              <IonCol>
                {" "}
                <IonText color="primary">
                  {new Date(parkingOffer.start).getDate()}/
                  {new Date(parkingOffer.start).getMonth() + 1}/
                  {new Date(parkingOffer.start).getFullYear()}{" "}
                  {new Date(parkingOffer.start).getHours()}:
                  {new Date(parkingOffer.start).getMinutes()}
                </IonText>
              </IonCol>
            </IonRow>
            <IonRow>
              <IonCol>
                <IonText color="primary">תאריך סיום</IonText>
              </IonCol>
              <IonCol>
                {" "}
                <IonText color="primary">
                  {new Date(parkingOffer.end).getDate()}/
                  {new Date(parkingOffer.end).getMonth() + 1}/
                  {new Date(parkingOffer.end).getFullYear()}{" "}
                  {new Date(parkingOffer.end).getHours()}:
                  {new Date(parkingOffer.end).getMinutes()}
                </IonText>
              </IonCol>
            </IonRow>
            <IonRow>
              <IonCol>
                <IonText color="primary">גודל חניה</IonText>
              </IonCol>
              <IonCol>
                {parkingOffer.parking.size === "Big" && (
                  <IonText color="primary">גדולה</IonText>
                )}
                {parkingOffer.parking.size === "Small" && (
                  <IonText color="primary">קטנה</IonText>
                )}
              </IonCol>
            </IonRow>
            <IonRow>
              <IonCol>
                <IonText color="primary">מחיר</IonText>
              </IonCol>
              <IonCol>
            ...
            {parkingOffer.status == "Open" && parkingOffer.parking.isPrivate && (
              <IonRow className={"paypal-row"}>
                <Paypal
                  price={parkingOffer.price}
                  merchantId={merchantId}
                  parkingOfferId={parkingOffer.id}
                />
              </IonRow>
            )}
          </IonGrid>
          {!parkingOffer.parking.isPrivate && (
            <>
              ...
              <br></br>
              <IonButtons>
                <IonButton className="innerText" onClick={acceptOffer}>
                  קבל הצעה
                </IonButton>
              </IonButtons>
            </>
