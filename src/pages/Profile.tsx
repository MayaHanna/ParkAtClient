...
import {
  fullParkingsOffersWithOwnerSelector,
  fullParkingsOffersWithClientSelector,
} from "../data/parkings-offers-module/selectors";
...
const Profile: React.FC = () => {
  ...
  const [currentUserPayPal, setCurrentUserPayPal] = useState(
  ...
  useEffect(() => {
    if (user.userMailAddress == undefined) return setIsLoading(true);
  ...
  const parkingOffers: FullParkingOffer[] = useSelector((state: RootState) =>
    fullParkingsOffersWithOwnerSelector(state, user.userMailAddress)
  );
  const parkingHistory: FullParkingOffer[] = useSelector((state: RootState) =>
    fullParkingsOffersWithClientSelector(state, user.userMailAddress)
  );
          ...
          <IonContent fullscreen>
            <div className={"profile-title"}>
              ...
              <div className={"profile-title-details"}>
                ...
                <IonText color="primary" className={"user-mail"}>
                ...
                {isEditUser ? (
                  <>
                    <IonInput
                      ...
                      type="text"
                      name="merchantId"
                      value={currentUserPayPal}
                      onIonChange={(e) => setCurrentUserPayPal(e.detail.value!)}
                      placeholder={"הכנס חשבון PayPal כאן"}
                    />
                    <IonButtons>
                      <IonButton
                        className="innerText"
                        onClick={onPayPalAccountChanged}
                      >
                        שמור
                      </IonButton>
                    </IonButtons>
                  ...
                  <IonText color="primary">
                    {userMerchant.merchantId || "הכנס חשבון PayPal כאן"}
                  </IonText>
                )}
              </div>
            )}
