...
import {
  FullParkingOffer,
  ParkingOffersMapParams,
} from "../data/parkings-offers-module/types";
...
const MapWrapper: React.FC<MapWrapperProps> = ({ filterParams }) => {
  ...
  const parkingOffers: FullParkingOffer[] = useSelector((state: RootState) =>
    fullParkingsOffersWithFilterSelector(state, filterParams).filter(
      (po) => po.status === "Open"
    )
  );
  const [center, setCenter] = useState<Coords>(
    filterParams?.centerLocation ? filterParams?.centerLocation : initialCenter
  );
  ...
  useEffect(() => {
    setCenter(
      filterParams?.centerLocation ? filterParams?.centerLocation : center
    );
        ...
        <Marker
          ...
          onClick={(e) =>
            history.push(
              `/parkingOffer/${
                po.id
              }?canAddComment=${false}&canAddImage=${false}`
            )
          }
          icon={
